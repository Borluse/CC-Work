...
- 主题根 `总览.md` 只维护跨需求的当前摘要；`index.yaml` 是编号、状态、里程碑、文档路径和 Review 关系的权威来源；详细方案、证据和修复过程留在需求点、Bug、Review 文档。
- 日常定位、实现、Bug、Review 和状态汇总默认不整读 `总览.md`：先用账本、检索摘要和目标文档定位；只有里程碑、依赖、跨需求决策或未闭合风险无法从账本和相关文档确定时，才按标题读取总览相关章节。
- 新增或拆分需求时，优先读取总览中与新点有关的目标、技术约束、系统盘点、跨需求依赖和未闭合问题；无交互的需求点详情和历史变更记录默认跳过。用户明确要求完整盘点时才整读总览。
- 整合或导出 Wiki 时，总览只作为当前跨需求摘要和关键决策参考；事实以 `index.yaml`、相关设计/Bug/Review 文档和代码为准。历史变更记录仅在核对决策演进或处理冲突时读取。
- 生成或刷新总览时，只保留当前目标、有效约束、系统关系、简明需求清单、跨需求依赖和未闭合问题。每个需求点在清单中保留一行；需求点章节只写简短摘要或跨需求关键决策，不复制详细设计、实现核验和修复过程；已有章节就地更新，不追加重复章节或重复依赖图。
- 同一会话内已读取的总览章节不重复读取；目标范围变化、用户点名或出现冲突时再补读。
