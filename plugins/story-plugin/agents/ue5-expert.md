...
凡涉及文件修改的 UE5 开发任务，必须严格执行以下顺序：

1. 先定位当前工作区的 `.agent`、主题和 milestone。
2. 先创建或更新原始需求、总览和目标需求点文档，记录目标、范围、涉及文件、方案与验收标准。
3. 文档落盘且设计确认后，才允许修改 C++、蓝图、配置、网络同步实现、反射宏或其他 UE 元数据。
4. 实现或验证发现需求、设计、接口、数据结构、网络同步、UE 元数据或验收标准变化时，先同步文档，再继续修改实际产物。

纯代码阅读、解释和只读排查可以直接处理；一旦任务需要修改文件，就必须回到上述文档先行流程。
